#!/usr/bin/env python3
"""
synthesize_10min.py - Synthesize GAA Kickout Analysis Results from First 10 Minutes
Processes analysis results from analyze_first_10min.py into comprehensive JSON
"""
...
def synthesize_kickouts_with_ai(analysis_results):
    ...
    for result in analysis_results:
        all_analyses += f"\n--- FIRST 10 MINUTES - {result['timestamp']} (starts at {result['clip_start_time']}s) ---\n"
        all_analyses += result['content']
        all_analyses += "\n"
    
    prompt = f"""
You are an expert GAA analyst synthesizing kickout data from {len(analysis_results)} clip analyses from the first 10 minutes of the match.
...
4. Calculate kickout effectiveness statistics
...
- Track tactical patterns
- Provide comprehensive statistics

OUTPUT FORMAT (JSON):
{{
  "match_info": {{
    "title": "GAA Match - First 10 Minutes Kickout Analysis",
    "description": "AI-synthesized kickout timeline from first 10 minutes",
    "total_kickouts": [number],
    "analysis_method": "Enhanced AI video analysis",
    "period_analyzed": "First 10 minutes"
  }},
  "kickouts": [
    {{
  ...
  "tactical_insights": {{
    "most_active_period": "[time period with most kickouts]",
...
def save_results(kickout_data, output_dir):
    ...
    if 'kickouts' in kickout_data:
        timeline_data = {
            "title": "GAA Match - First 10 Minutes Kickout Timeline",
    ...
    with open(summary_file, 'w') as f:
        f.write("GAA KICKOUT ANALYSIS SUMMARY - FIRST 10 MINUTES\n")
        f.write("=" * 60 + "\n\n")
        ...
        if 'kickouts' in kickout_data:
            ...
            for kickout in kickout_data['kickouts']:
                ...
                f.write(f"  {time_str} - {kickout['team']}\n")
    
    print(f"💾 Summary saved: {summary_file}")

def main():
    print("🥅 GAA KICKOUT SYNTHESIS - FIRST 10 MINUTES")
    print("=" * 60)
    print("Synthesizing kickout analysis from first 10 minutes")
    print("=" * 60)
    
    analysis_dir = "../results/kickout_first_10min"
    output_dir = "../results/kickout_synthesis_10min"
    
    # Collect all analysis results
    print(f"📋 Collecting analysis results from {analysis_dir}...")
    ...
    save_results(kickout_data, output_dir)
